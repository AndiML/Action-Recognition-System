...
def get_command_descriptors() -> list[BaseCommandDescriptor]:
    ...
    for module_path in glob(os.path.join(os.path.dirname(__file__), '**', 'command_descriptor.py')):
        ...
        command_descriptor_modules.append(importlib.import_module(f'action_recognition.commands.{sub_package_name}.command_descriptor'))
...
def get_command(command_name: str) -> type:
    ...
    command_module = importlib.import_module(f'action_recognition.commands.{command_module_name}.command')

    # Gets the class of the command, the name of the command class is in pascal case
